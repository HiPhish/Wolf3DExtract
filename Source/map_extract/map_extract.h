 ...
 *  
 *  These codes are returned by the extraction methods; they can then be mapped
 *  by another function to handle the error appropriately instead of just
 *  aborting.
 */
enum map_error {
...
/** Structure describing how to find the individual levels in the GAMEAPS
 *  file.*/
struct level_atlas {
 ...
 *
 *  The levels are too large to be all loaded into memory at the same time,
 *  instead these headers can be used to find the individual maps and assemble
 *  the level.
 */
struct level_header {
 ...
 *
 *  @return  Size of the allocated buffer if everything went right, zero
 *           otherwise.
 *
 *  The buffer does not need to be pre-allocated, the function will free its
 *  memory and allocate enough space. If an error occurs the buffer will not get
 *  changed.
 */
