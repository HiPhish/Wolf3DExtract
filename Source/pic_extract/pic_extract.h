...
//Error codes for picture extraction. These codes are returned by the extraction
//functions; they can then be mapped by another function to handle the error
//appropriately instead of just aborting.
 ...
 *
 *  The array of texel bytes is not stored in the struct, instead it's pointed
 *  at. The array size varies from pic to pic, so it can't be stored inside the
 *  struct.
 */
struct picture {
 ...
 *
 *  @return  Amount of allocated memory, 0 of an error occurs, size of a picture
 *  struct otherwise.
 *
 *  The buffer does not have to be allocated, it will be freed and re-allocated
 *  by this function.
 */
