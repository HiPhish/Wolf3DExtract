/** @file wolf3dextract.c
 *  
 *  This is the main program file of the Wolfenstein 3D extractor tool. It
 *  combines the various modules, coordinates interaction between them and
 *  processes the users input to decide what to do.
 *
 *  This file has no accompaigning header files because none of the
 *  declarations are used outside. All declarations are contained here.
 */
...
/*-[ CONSTANTS ]--------------------------------------------------------------*/
/* Define macros for the command-line options and exit codes in case I decide */
...
/** Various exit codes for the applications. */
enum exit_codes {
	SUCCESS         , //< No error.
	INVALID_EPISODE , //< There is no such episode in the game.
	INVALID_LEVEL   , //< There is no such level in the episode.
	INVALID_ARGS    , //< Invalid command-line arguments.
	INVALID_FILES   , //< Resource files either don't exist or can't be read.
	INVALID_MAP     , //< There is no such map in the game.
	
	PROGRAM_ERRORS    //< Number of exit codes
};


/*-[ MACROS ]-----------------------------------------------------------------*/
/* These macros are used for string comparison when handling command-line     */
/* parameters. It's easier to read than the code they replace.                */
/*----------------------------------------------------------------------------*/
...
/** Processe command-line arguments one after the other.
 *
 ...
 *
 * This function is usually called when the user passed no arguments or invalid
 * ones. The recommended output file is the standard error.
 */
 ...
 *
 * Both `episode` and `level` are strings and get converted to integers by the
 * function.
 */
 ...
 *
 * All three parameters `episode`, `level` and `map` are strings and get
 * converted to integers by the function.
 */
 ...
 *
 * The table elements are pairs of signed 16-bit integers, where the first
 * number of an element is the width and the second one is the height.
 */
...
void print_usage(FILE *file) {
	// each command-line option is up to three characters long, keep that in
	// mind for the text width
	fprintf(file, "Usage: Call from the same directory where your data files are located and pass\n"
			      ...
			      "The output is printed to the standard output, so you'll want to redirect it into\n"
			      "another file or pipe it into another program.\n"
			      "Arguments are processed in the order they are given, so if for exapmle you want\n"
				  "to specify the extension you have to do it before trying to extract an asset.\n"
			);
}
